...
let mode = 'light'; // default to light physics
let ammo = null;
...
export async function initPhysics(modeChoice = 'light') {
  ...
  try {
    ...
    if (enabled) {
      // instantiate already-registered bodies in Ammo world
...
export function registerBody(body) {
  ...
  if (mode === 'ammo' && ammo && ammo.enabled) {
    try {
      ammo.createBody(body);
    } catch (e) {}
  } else {
    try {
      light.registerBody(body);
    } catch (e) {}
  }
}
...
export function applyForce(body, force) {
  ...
  if (mode === 'ammo' && ammo && ammo.enabled) {
    try {
      ammo.applyForce(body, force);
    } catch (e) {}
  } else {
    try {
      if (typeof body.applyForce === 'function') body.applyForce(force);
    } catch (e) {}
  }
}
...
export function applyTorque(body, torque) {
  ...
  if (mode === 'ammo' && ammo && ammo.enabled) {
    try {
      ammo.applyTorque(body, torque);
    } catch (e) {}
  } else {
    try {
      if (typeof body.applyTorque === 'function') body.applyTorque(torque);
    } catch (e) {}
  }
}
