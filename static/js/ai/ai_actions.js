...
export async function applyAIActions(text, editorInstance) {
  ...
  if (lower.includes('spawn aircraft') || lower.match(/spawn\s+(aircraft|plane|airplane)/i)) {
    ...
    // create entity (use registry + GLB validation to avoid corrupted GLBs)
    let ent = null;
    try {
      let useModel = false;
      try {
      ...
      if (useModel) {
        try {
          ...
          const ok = await validateGlbHeader('/static/assets/models/aircraft/airplane.glb');
          if (ok) {
            ent = viewer.entities.add({ id: `aircraft-${Date.now()}`, name: 'aircraft', position: spawnPos, model: { uri: '/static/assets/models/aircraft/airplane.glb' } });
            if (ent.model && ent.model.readyPromise && typeof ent.model.readyPromise.then === 'function') {
              try { await ent.model.readyPromise; } catch (err) { try { viewer.entities.remove(ent); } catch (e) {} ent = viewer.entities.add({ id: `aircraft-${Date.now()}`, name: 'aircraft', position: spawnPos, point: { pixelSize: 12, color: Cesium.Color.WHITE } }); }
            }
          } else {
            ent = viewer.entities.add({ id: `aircraft-${Date.now()}`, name: 'aircraft', position: spawnPos, point: { pixelSize: 12, color: Cesium.Color.WHITE } });
          }
        } catch (e) {
          ent = viewer.entities.add({ id: `aircraft-${Date.now()}`, name: 'aircraft', position: spawnPos, point: { pixelSize: 12, color: Cesium.Color.WHITE } });
        }
