export class AmmoAdapter {
  ...
  createBody(appBody) {
    ...
    // Expose applyForce/applyTorque on the application body so callers work in Ammo mode
    try {
      appBody.applyForce = (f) => this.applyForce(appBody, f);
      appBody.applyTorque = (t) => this.applyTorque(appBody, t);
    } catch (e) {}
  ...
  applyForce(appBody, force) {
    if (!this.enabled) return;
    const rec = this._bodies.get(appBody);
    if (!rec) return;
    const Ammo = this.Ammo;
    try {
      const f = new Ammo.btVector3(force.x || 0, force.y || 0, force.z || 0);
      rec.body.applyCentralForce(f);
      Ammo.destroy(f);
    } catch (e) {
      console.warn('AmmoAdapter.applyForce failed', e);
    }
  }

  applyTorque(appBody, torque) {
    if (!this.enabled) return;
    const rec = this._bodies.get(appBody);
    if (!rec) return;
    const Ammo = this.Ammo;
    try {
      const t = new Ammo.btVector3(torque.x || 0, torque.y || 0, torque.z || 0);
      rec.body.applyTorque(t);
      Ammo.destroy(t);
    } catch (e) {
      console.warn('AmmoAdapter.applyTorque failed', e);
    }
  }
